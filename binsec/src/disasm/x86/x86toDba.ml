...
let assign_xmm_zero ~dst off1 off2 ~src off3 off4 xmm mm sreg =
  ...
  match lhs with
  | Dba.LValue.Store _ ->
    [ Predba.assign lhs @@ Dba.Expr.restrict off1 off2 rhs ]
...
let lift_mv_seg_left_gen ~dst ~rv =
  ...
  let index =
    Expr.logand rv @@
      Expr.constant (Bitvector.create (Bigint.big_int_of_int 0xfffc) 16) in
  ...
  let bits0_23 =
    let chunk =
      Expr.logand desc_rv
        (Expr.constant (Bitvector.create (Bigint.big_int_of_int64
          (Int64.of_string "0x00_00_00_ff_ff_ff_00_00")) 64))
    in
    Expr.shift_right chunk (Expr.constant (Bitvector.create (Bigint.big_int_of_int 16) 64))
    |> Expr.restrict 0 23
  in
  ...
  let bits24_31 =
    let chunk =
      Expr.logand desc_rv
        (Expr.constant (Bitvector.create (Bigint.big_int_of_int64
          (Int64.of_string "0xff_00_00_00_00_00_00_00")) 64))
    in
    Expr.shift_right chunk (Expr.constant (Bitvector.create (Bigint.big_int_of_int 56) 64))
    |> Expr.restrict 0 7
  in
