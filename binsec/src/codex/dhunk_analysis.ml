...
module Dhunk_regex = Regex.Make(G.E)
module Reduce = Reduce.Make(G)(Dhunk_regex)
module Wto_alg = Wto.Make(G.V)
...
let graph_of_dhunk dhunk : G.t * (Dba.id * Virtual_address.t option) list =
  ...
  let next = function
    ...
    | NondetAssume (_,_,nxt) -> [Jump_Inner nxt]
    | Nondet (_,_,nxt) -> [Jump_Inner nxt]
    | Undef (_,nxt) -> [Jump_Inner nxt]
    | Malloc (_,_,nxt) -> [Jump_Inner nxt]
    | Free (_,nxt) -> [Jump_Inner nxt]
    | Print (_,nxt) -> [Jump_Inner nxt]
  in
