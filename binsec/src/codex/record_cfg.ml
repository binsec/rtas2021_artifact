...
module type S = sig
  ...
  val end_fork: t -> unit
  val close: t -> Virtual_address.t -> string -> Cfg_analysis.Cfg.t
...
: S = struct
  ...
  let dump_graph trace entry_node bbg block_map filename =
  ...
  let close tr initial_addr graph_filename =
    ...
    dump_graph tr (initial_addr, []) bbg block_map graph_filename;
    bbg
