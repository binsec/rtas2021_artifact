...
let type_size =
  ...
  | Void | Function _ | Structure {st_byte_size=None;_} | Array _ ->
...
let subtype symbols ((t,i) : cell_typ) ((u,j) : cell_typ) =
  ...
  let open Ctypes in
  if Ctypes.equal_descr ~only_descr:false t.descr u.descr && i = j then
    if not t.const && u.const then false
    else u.pred = Pred.True || t.pred = u.pred
  else
    if not t.const && u.const then false
    else if not (u.pred = Pred.True || t.pred = u.pred) then false
    else
      let rec parents x ofs =
        try
          let parent_t, parent_ofs = type_at_offset symbols x.descr ofs in
...
module Make (Memory : MEMORY) = struct
  ...
  let array_size symbols elem_t length =
    ...
    Z.mul (eval_value symbols length) (Z.of_int @@ type_size elem_t)
  ...
  let type_cell a (typ : cell_typ) (typing, symbols, to_check) =
    ...
    | t, (u,_) when Ctypes.equal ~only_descr:false t u ->
    ...
    *)
    ...
    and propagate_type path ~mutate (typ : Ctypes.typ) a =
      ...
      | Ptr{pointed = {descr=Array (elem_t, Some ar_sz);_} as pointed_t; index = PastEnd} ->
          if mutate then type_region a typ ptr_size state;
          let size = array_size symbols elem_t.descr ar_sz in
          check_region path a typ |> iter_error;
          let v = Memory.read_u32 mem a in
          if v <> 0 then
      ...
      | Ptr{pointed = {descr=Array (_, Some _);_} as pointed; index = ValidUnknown} ->
          ...
          | (t,_) when Ctypes.equal ~only_descr:false t pointed ->
              ()
          | _ ->
      ...
      | Array (elem_t, Some sz) ->
          ...
          let elem_sz = type_size elem_t.descr in
          let ar_size = array_size symbols elem_t.descr sz in
