...
#ifndef __MSHRCONTAINER_H__
#define __MSHRCONTAINER_H__
...
public:
    ...
    bool AddRead(address_t _address, address_t _base, int _index, int _size, int _port,
        ...
        case MSHR_ENTRY_FREE:
            ...
            set = _set;
            line = _line;
            read_entry[0].Init(_address, _base, _index, _size, _port, _tag, _instruction);
            instruction = _instruction;
            return true;
            break;
        case MSHR_ENTRY_WAITING_DATA:
...
//      index = _address & (((address_t)nLineSize) - 1);
            read_entry[read_entry_index].InitWithData(_address, _base, _index, _size, _port,
                    _tag, _instruction,
                    &line_data[index], &line_valid[index]);
            return true;
            break;
        case MSHR_ENTRY_SERVICING_READS:
            return false;
...
public:
    ...
    void Update(address_t addr, uint8_t *in_data,
                int *locked_set, int *locked_line, bool *dirty, uint8_t *out_data) {
        ...
        if (!MSHRQueue.Empty()) {
            ...
            if (mshrit) {
                if (mshrit->AddRead(addr, base, index, size,
                                    port, tag,
                                    instruction, lock_set, lock_line)) {
                    if (found) {
                        return EXISTENT_MSHR_ENTRY;
                    } else {
    ...
    bool HasAddress(address_t address)
    { if (MSHRQueue.Empty()) return false;
      OoOQueuePointer<MSHREntry<T, nSources, nLineSize, nCPULineSize, nMSHRRead>, nMSHR> mshrit;
      mshrit = MSHRQueue.SeekAtHead();
      while (mshrit) {
        if(mshrit->HasAddress(address)) return true;
        mshrit++;
      }
      return false;
    }
