...
UNISIM_SIMULATORS_DIR=${MY_DIR}/../unisim_simulators/tlm2/ppc440emu_system

PPC440EMU_SYSTEM_VERSION=$(cat ${UNISIM_SIMULATORS_DIR}/VERSION)
GENISSLIB_VERSION=$(cat ${UNISIM_TOOLS_DIR}/genisslib/VERSION)-ppc440emu_system-${PPC440EMU_SYSTEM_VERSION}
...
unisim/service/loader/ppc_uboot/ppc_uboot.hh \
unisim/service/loader/ppc_linux_kernel_loader/ppc_linux_kernel_loader.hh \
...
unisim/service/loader/ppc_uboot/ppc_uboot.tcc \
unisim/service/loader/ppc_linux_kernel_loader/ppc_linux_kernel_loader.tcc \
...
UNISIM_LIB_PPC440EMU_SYSTEM_DATA_FILES="\
unisim/service/debug/gdb_server/gdb_powerpc.xml"

PPC440EMU_SYSTEM_EXTERNAL_HEADERS="\
assert.h \
ctype.h \
cxxabi.h \
errno.h \
...
UNISIM_SIMULATORS_PPC440EMU_SYSTEM_TEMPLATE_FILES=
UNISIM_SIMULATORS_PPC440EMU_SYSTEM_DATA_FILES="\
COPYING \
INSTALL \
NEWS \
README \
...
UNISIM_SIMULATORS_PPC440EMU_SYSTEM_TESTBENCH_FILES=""

has_to_build_configure=no
has_to_build_genisslib_configure=no
has_to_build_ppc440emu_system_configure=no

mkdir -p ${DEST_DIR}/genisslib
mkdir -p ${DEST_DIR}/ppc440emu_system
...
UNISIM_LIB_PPC440EMU_SYSTEM_FILES="${UNISIM_LIB_PPC440EMU_SYSTEM_SOURCE_FILES} ${UNISIM_LIB_PPC440EMU_SYSTEM_HEADER_FILES} ${UNISIM_LIB_PPC440EMU_SYSTEM_TEMPLATE_FILES} ${UNISIM_LIB_PPC440EMU_SYSTEM_DATA_FILES}"

for file in ${UNISIM_LIB_PPC440EMU_SYSTEM_FILES}; do
	mkdir -p "${DEST_DIR}/ppc440emu_system/`dirname ${file}`"
	has_to_copy=no
	if [ -e "${DEST_DIR}/ppc440emu_system/${file}" ]; then
		if [ "${UNISIM_LIB_DIR}/${file}" -nt "${DEST_DIR}/ppc440emu_system/${file}" ]; then
			has_to_copy=yes
		fi
	else
		has_to_copy=yes
	...
	if [ "${has_to_copy}" = "yes" ]; then
		echo "${UNISIM_LIB_DIR}/${file} ==> ${DEST_DIR}/ppc440emu_system/${file}"
		cp -f "${UNISIM_LIB_DIR}/${file}" "${DEST_DIR}/ppc440emu_system/${file}" || exit
	fi
done

UNISIM_SIMULATORS_PPC440EMU_SYSTEM_FILES="${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_SOURCE_FILES} ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_HEADER_FILES} ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_TEMPLATE_FILES} ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_DATA_FILES} ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_TESTBENCH_FILES}"

for file in ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_FILES}; do
	has_to_copy=no
	if [ -e "${DEST_DIR}/ppc440emu_system/${file}" ]; then
		if [ "${UNISIM_SIMULATORS_DIR}/${file}" -nt "${DEST_DIR}/ppc440emu_system/${file}" ]; then
			has_to_copy=yes
		fi
	else
		has_to_copy=yes
	...
	if [ "${has_to_copy}" = "yes" ]; then
		echo "${UNISIM_SIMULATORS_DIR}/${file} ==> ${DEST_DIR}/ppc440emu_system/${file}"
		cp -f "${UNISIM_SIMULATORS_DIR}/${file}" "${DEST_DIR}/ppc440emu_system/${file}" || exit
	fi
done

for file in ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_DATA_FILES}; do
	has_to_copy=no
...
mkdir -p ${DEST_DIR}/ppc440emu_system/config
mkdir -p ${DEST_DIR}/ppc440emu_system/m4
...
for file in ${UNISIM_LIB_PPC440EMU_SYSTEM_M4_FILES}; do
	has_to_copy=no
	if [ -e "${DEST_DIR}/ppc440emu_system/${file}" ]; then
		if [ "${UNISIM_LIB_DIR}/${file}" -nt  "${DEST_DIR}/ppc440emu_system/${file}" ]; then
			has_to_copy=yes
	...
	if [ "${has_to_copy}" = "yes" ]; then
		echo "${UNISIM_LIB_DIR}/${file} ==> ${DEST_DIR}/ppc440emu_system/${file}"
		cp -f "${UNISIM_LIB_DIR}/${file}" "${DEST_DIR}/ppc440emu_system/${file}" || exit
		has_to_build_ppc440emu_system_configure=yes
	fi
done
...
echo "  - UNISIM ppc440emu_system: a user level PowerPC SystemC TLM simulator." >> "${DEST_DIR}/README"
...
if [ "${has_to_build_configure}" = "yes" ]; then
	...
	echo "AC_INIT([UNISIM PowerMAC G4 PCI Standalone simulator], [${PPC440EMU_SYSTEM_VERSION}], [Gilles Mouchard <gilles.mouchard@cea.fr>, Daniel Gracia Perez <daniel.gracia-perez@cea.fr>, Reda Nouacer <reda.nouacer@cea.fr>], [unisim-ppc440emu_system])" > "${DEST_DIR}/configure.ac"
	...
	echo "AC_CONFIG_SUBDIRS([ppc440emu_system])"  >> "${CONFIGURE_AC}" 
	echo "AC_CONFIG_FILES([Makefile])" >> "${CONFIGURE_AC}"
	echo "AC_OUTPUT" >> "${CONFIGURE_AC}"

	echo "Generating Makefile.am"
	echo "SUBDIRS=genisslib ppc440emu_system" > "${MAKEFILE_AM}"
...
PPC440EMU_SYSTEM_CONFIGURE_AC="${DEST_DIR}/ppc440emu_system/configure.ac"
PPC440EMU_SYSTEM_MAKEFILE_AM="${DEST_DIR}/ppc440emu_system/Makefile.am"


if [ ! -e "${PPC440EMU_SYSTEM_CONFIGURE_AC}" ]; then
	has_to_build_ppc440emu_system_configure=yes
else
	if [ "$0" -nt "${PPC440EMU_SYSTEM_CONFIGURE_AC}" ]; then
		has_to_build_ppc440emu_system_configure=yes
	fi
fi

if [ ! -e "${PPC440EMU_SYSTEM_MAKEFILE_AM}" ]; then
	has_to_build_ppc440emu_system_configure=yes
else
	if [ "$0" -nt "${PPC440EMU_SYSTEM_MAKEFILE_AM}" ]; then
		has_to_build_ppc440emu_system_configure=yes
	fi
fi

if [ "${has_to_build_ppc440emu_system_configure}" = "yes" ]; then
	echo "Generating ppc440emu_system configure.ac"
	echo "AC_INIT([UNISIM PowerMac G4 PCI C++ simulator], [${PPC440EMU_SYSTEM_VERSION}], [Gilles Mouchard <gilles.mouchard@cea.fr>, Daniel Gracia Perez <daniel.gracia-perez@cea.fr>, Reda Nouacer <reda.nouacer@cea.fr>], [unisim-ppc440emu_system-core])" > "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_CONFIG_MACRO_DIR([m4])" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_CONFIG_AUX_DIR(config)" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_CONFIG_HEADERS([config.h])" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_CANONICAL_BUILD" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_CANONICAL_HOST" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_CANONICAL_TARGET" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AM_INIT_AUTOMAKE([subdir-objects tar-pax])" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_PATH_PROGS(SH, sh)" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_PROG_CXX" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_PROG_RANLIB" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_PROG_INSTALL" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_PROG_LN_S" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_LANG([C++])" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AM_PROG_CC_C_O" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_CHECK_HEADERS([${PPC440EMU_SYSTEM_EXTERNAL_HEADERS}],, AC_MSG_ERROR([Some external headers are missing.]))" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "UNISIM_CHECK_TIMES" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "UNISIM_CHECK_ENDIAN" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "UNISIM_CHECK_CURSES" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "UNISIM_CHECK_LIBEDIT" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "UNISIM_CHECK_BSD_SOCKETS" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "UNISIM_CHECK_ZLIB" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	...
	echo "UNISIM_CHECK_GET_EXECUTABLE_PATH" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "UNISIM_CHECK_REAL_PATH" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "GENISSLIB_PATH=\`pwd\`/../genisslib/genisslib" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_SUBST(GENISSLIB_PATH)" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_DEFINE([BIN_TO_SHARED_DATA_PATH], [\"../share/unisim-ppc440emu_system-${PPC440EMU_SYSTEM_VERSION}\"], [path of shared data relative to bin directory])" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_CONFIG_FILES([Makefile])" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"
	echo "AC_OUTPUT" >> "${PPC440EMU_SYSTEM_CONFIGURE_AC}"

	AM_PPC440EMU_SYSTEM_VERSION=`printf ${PPC440EMU_SYSTEM_VERSION} | sed -e 's/\./_/g'`
	echo "Generating ppc440emu_system Makefile.am"
	echo "ACLOCAL_AMFLAGS=-I \$(abs_top_srcdir)/m4" > "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "INCLUDES=-I\$(abs_top_srcdir) -I\$(abs_top_builddir)" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "noinst_LIBRARIES = libppc440emu_system-${PPC440EMU_SYSTEM_VERSION}.a" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "libppc440emu_system_${AM_PPC440EMU_SYSTEM_VERSION}_a_SOURCES = ${UNISIM_LIB_PPC440EMU_SYSTEM_SOURCE_FILES}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "bin_PROGRAMS = unisim-ppc440emu_system-${PPC440EMU_SYSTEM_VERSION} unisim-ppc440emu_system-debug-${PPC440EMU_SYSTEM_VERSION} unisim-ppc440emu_system-wfpu-${PPC440EMU_SYSTEM_VERSION} unisim-ppc440emu_system-wfpu-debug-${PPC440EMU_SYSTEM_VERSION}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_${AM_PPC440EMU_SYSTEM_VERSION}_SOURCES = ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_SOURCE_FILES}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_wfpu_${AM_PPC440EMU_SYSTEM_VERSION}_SOURCES = ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_SOURCE_FILES}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_debug_${AM_PPC440EMU_SYSTEM_VERSION}_SOURCES = ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_SOURCE_FILES}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_wfpu_debug_${AM_PPC440EMU_SYSTEM_VERSION}_SOURCES = ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_SOURCE_FILES}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_wfpu_${AM_PPC440EMU_SYSTEM_VERSION}_CPPFLAGS = -DWITH_FPU" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_debug_${AM_PPC440EMU_SYSTEM_VERSION}_CPPFLAGS = -DDEBUG_PPC440EMU_SYSTEM" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_wfpu_debug_${AM_PPC440EMU_SYSTEM_VERSION}_CPPFLAGS = -DWITH_FPU -DDEBUG_PPC440EMU_SYSTEM" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_${AM_PPC440EMU_SYSTEM_VERSION}_LDADD = libppc440emu_system-${PPC440EMU_SYSTEM_VERSION}.a" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_wfpu_${AM_PPC440EMU_SYSTEM_VERSION}_LDADD = libppc440emu_system-${PPC440EMU_SYSTEM_VERSION}.a" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_debug_${AM_PPC440EMU_SYSTEM_VERSION}_LDADD = libppc440emu_system-${PPC440EMU_SYSTEM_VERSION}.a" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "unisim_ppc440emu_system_wfpu_debug_${AM_PPC440EMU_SYSTEM_VERSION}_LDADD = libppc440emu_system-${PPC440EMU_SYSTEM_VERSION}.a" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"

	echo "noinst_HEADERS = ${UNISIM_LIB_PPC440EMU_SYSTEM_HEADER_FILES} ${UNISIM_LIB_PPC440EMU_SYSTEM_TEMPLATE_FILES} ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_HEADER_FILES} ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_TEMPLATE_FILES}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "EXTRA_DIST = ${UNISIM_LIB_PPC440EMU_SYSTEM_M4_FILES}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "sharedir = \$(prefix)/share/unisim-ppc440emu_system-${PPC440EMU_SYSTEM_VERSION}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "dist_share_DATA = ${UNISIM_LIB_PPC440EMU_SYSTEM_DATA_FILES} ${UNISIM_SIMULATORS_PPC440EMU_SYSTEM_DATA_FILES}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"

	echo "BUILT_SOURCES=\$(abs_top_builddir)/unisim/component/cxx/processor/powerpc/ppc440/isa.hh \$(abs_top_builddir)/unisim/component/cxx/processor/powerpc/ppc440/isa.tcc" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "CLEANFILES=\$(abs_top_builddir)/unisim/component/cxx/processor/powerpc/ppc440/isa.hh \$(abs_top_builddir)/unisim/component/cxx/processor/powerpc/ppc440/isa.tcc" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "\$(abs_top_builddir)/unisim/component/cxx/processor/powerpc/ppc440/isa.tcc: \$(abs_top_builddir)/unisim/component/cxx/processor/powerpc/ppc440/isa.hh" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "\$(abs_top_builddir)/unisim/component/cxx/processor/powerpc/ppc440/isa.hh: ${UNISIM_LIB_PPC440EMU_SYSTEM_ISA_FILES}" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\t" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "\$(GENISSLIB_PATH) -o \$(abs_top_builddir)/unisim/component/cxx/processor/powerpc/ppc440/isa -w 8 -I \$(abs_top_srcdir) \$(abs_top_srcdir)/unisim/component/cxx/processor/powerpc/ppc440/isa/ppc440.isa" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"

	echo "all-local: all-local-bin all-local-share" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "clean-local: clean-local-bin clean-local-share" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "all-local-bin: \$(bin_PROGRAMS)" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\t@PROGRAMS='\$(bin_PROGRAMS)'; \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	...
	printf "\tdone\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "clean-local-bin:" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\t@if [ ! -z '\$(bin_PROGRAMS)' ]; then \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\trm -rf '\$(abs_top_builddir)/bin'; \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\tfi\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "all-local-share: \$(dist_share_DATA)" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\t@SHARED_DATAS='\$(dist_share_DATA)'; \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\tfor SHARED_DATA in \$\${SHARED_DATAS}; do \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\trm -f \"\$(abs_top_builddir)/share/unisim-ppc440emu_system-${PPC440EMU_SYSTEM_VERSION}/\`basename \$\${SHARED_DATA}\`\"; \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\tmkdir -p '\$(abs_top_builddir)/share/unisim-ppc440emu_system-${PPC440EMU_SYSTEM_VERSION}'; \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\t(cd '\$(abs_top_builddir)/share/unisim-ppc440emu_system-${PPC440EMU_SYSTEM_VERSION}' && cp -f \"\$(abs_top_srcdir)/\$\${SHARED_DATA}\" \`basename \"\$\${SHARED_DATA}\"\`); \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\tdone\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	echo "clean-local-share:" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\t@if [ ! -z '\$(dist_share_DATA)' ]; then \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\trm -rf '\$(abs_top_builddir)/share'; \\\\\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"
	printf "\tfi\n" >> "${PPC440EMU_SYSTEM_MAKEFILE_AM}"

	echo "Building powerpc configure"
	${SHELL} -c "cd ${DEST_DIR}/ppc440emu_system && aclocal -I m4 && autoconf --force && autoheader && automake -ac"
fi
