...
type ('arg, 'ret) term =
  ...
  | Bunion: Transfer_functions.Condition.t * size -> ((binary,binary) ar2,binary) term
  | Bchoose: Transfer_functions.Choice.t * size -> (binary ar1,binary) term
...
module Build:sig
  ...
  module Binary:sig
    ...
    val bunion: size:int -> Transfer_functions.Condition.t -> ((binary,binary) ar2, binary) term
  end
