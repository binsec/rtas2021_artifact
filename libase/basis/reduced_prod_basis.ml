...
module Make
  ...
  module Binary_Forward = struct
    ...
    let biadd = bop2_wrapped BF1.biadd BF2.biadd
    let bisub = bop2_wrapped BF1.bisub BF2.bisub
  ...
  module Binary_Backward = struct
    ...
    let bofbool ~size bool bin =
      match bin with
      | Bot -> Some Quadrivalent_Lattice.Bottom
      | NonBot (bin1,bin2) ->
          match BB1.bofbool ~size bool bin1, BB2.bofbool ~size bool bin2 with
          | x, None -> x
          | None, x -> x
          | Some b1, Some b2 ->
              Some (if b1 <> b2 then Quadrivalent_Lattice.Bottom else b1)
  ...
  let binary_to_known_bits ~size x =
    ...
    Single_value_abstraction.Known_bits.inter ~size kb1 kb2
end
