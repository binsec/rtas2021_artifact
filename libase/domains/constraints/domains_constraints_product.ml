...
module Make
  ...
  let inter (a1,b1) (a2,b2) = (A.inter a1 a2,B.inter b1 b2)
  (* let join (a1,b1) (a2,b2) = (A.join a1 a2,B.join b1 b2) *)

  (* We do not know the constraints. *)
  let to_constraint (a,b) =A.to_constraint a

  module Query = struct
    let boolean (a,b) x = 
      Quadrivalent_Lattice.inter (A.Query.boolean a x) (B.Query.boolean b x)
    ...
    module Boolean_Lattice = Quadrivalent_Lattice
  ...
  let fixpoint_step ~lvl  ~actuals (arg_doma,arg_domb) ~args (final_doma,final_domb) ~finals =
    let resa,conta = A.fixpoint_step ~lvl ~actuals arg_doma ~args final_doma ~finals in
    let resb,contb = B.fixpoint_step ~lvl ~actuals arg_domb ~args final_domb ~finals in
