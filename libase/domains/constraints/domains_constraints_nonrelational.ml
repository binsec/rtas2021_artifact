...
module type Value = Lattice_sig.Join_Semi_Lattice_With_Inter_Bottom
...
struct
  ...
  module M3 = struct
    include Okasakimap.Make(struct
        include Key
      end)
    let mk_pretty fkey fvalue fmt x =
      iter x (fun key value ->
...
= struct
  ...
  module Const_eval = Constraints_const_eval.Make(Constraints);;
  ...
  let to_constraint _ = assert false (* (Constraints.Build.Boolean.unknown ~level:(0));; *)
  ...
  let fixpoint_step ~lvl  ~actuals arg_dom ~args final_dom ~finals =
  ...
  let ar2_boolean_boolean_boolean f dom a b res = 
  ...
  let ar1_boolean_boolean f dom a res = 
    let va = BooleanMap.find a dom.boolean_map in
  ...
  let ar0_binary f dom res =
    let binary_map = BinaryMap.add res f dom.binary_map in
    { dom with binary_map }
  ;;

  let ar2_binary_binary_binary f dom a b res =
  ...
  let ar2_binary_binary_boolean f dom a b res =
    let va = BinaryMap.find a dom.binary_map in
    let vb = BinaryMap.find b dom.binary_map in
    let boolean_map = BooleanMap.add res (f va vb) dom.boolean_map in
    { dom with boolean_map }
  ...
  let ar1_binary_binary f dom a res =
    let va = BinaryMap.find a dom.binary_map in
  ...
  module Binary_Forward = struct
    let biconst ~size k = ar0_binary (B.Binary_Forward.biconst ~size k)
    let beq    ~size = ar2_binary_binary_boolean @@ B.Binary_Forward.beq ~size
    let biule  ~size = ar2_binary_binary_boolean @@ B.Binary_Forward.biule ~size
    let bisle  ~size = ar2_binary_binary_boolean @@ B.Binary_Forward.bisle ~size
    let biadd  ~size ~nsw ~nuw = ar2_binary_binary_binary @@ B.Binary_Forward.biadd ~size ~nsw ~nuw
    let bisub  ~size ~nsw ~nuw = ar2_binary_binary_binary @@ B.Binary_Forward.bisub ~size ~nsw ~nuw
    let bimul  ~size ~nsw ~nuw = ar2_binary_binary_binary @@ B.Binary_Forward.bimul ~size ~nsw ~nuw
    let bshl   ~size ~nsw ~nuw = ar2_binary_binary_binary @@ B.Binary_Forward.bshl ~size ~nsw ~nuw
    let bxor   ~size = ar2_binary_binary_binary @@ B.Binary_Forward.bxor ~size
    let bor    ~size = ar2_binary_binary_binary @@ B.Binary_Forward.bor ~size
    let band   ~size = ar2_binary_binary_binary @@ B.Binary_Forward.band ~size
    let bashr  ~size = ar2_binary_binary_binary @@ B.Binary_Forward.bashr ~size
    let blshr  ~size = ar2_binary_binary_binary @@ B.Binary_Forward.blshr ~size
    let bisdiv ~size = ar2_binary_binary_binary @@ B.Binary_Forward.bisdiv ~size
    let biudiv ~size = ar2_binary_binary_binary @@ B.Binary_Forward.biudiv ~size
    let bismod ~size = ar2_binary_binary_binary @@ B.Binary_Forward.bismod ~size
    let biumod ~size = ar2_binary_binary_binary @@ B.Binary_Forward.biumod ~size

    let bsext ~size ~oldsize = ar1_binary_binary @@ B.Binary_Forward.bsext ~size ~oldsize
    let buext ~size ~oldsize = ar1_binary_binary @@ B.Binary_Forward.buext ~size ~oldsize
    let bchoose ~size cond = ar1_binary_binary @@ B.Binary_Forward.bchoose ~size cond
    let bofbool ~size = ar1_boolean_binary @@ B.Binary_Forward.bofbool ~size
    let bconcat ~size1 ~size2 = ar2_binary_binary_binary @@ B.Binary_Forward.bconcat ~size1 ~size2
    let bextract ~size ~index ~oldsize = ar1_binary_binary @@ B.Binary_Forward.bextract ~size ~index ~oldsize
  ...
  let binary_empty ~size = ar0_binary @@ B.Binary_Lattice.bottom ~size
  let integer_empty = ar0_integer B.Integer_Lattice.bottom
  let boolean_empty = ar0_boolean B.Boolean_Lattice.bottom        

  let binary_unknown ~size = ar0_binary (B.Binary_Lattice.top ~size)
  ...
  module Backward_Interp = struct
    ...
    let backward_interp dom (Constraints.Any x)=
      ...
      | Binary{term=T2{tag=Biadd{size;nsw;nuw};a;b}} ->  ar2_binary_binary_binary dom a b x @@ B.Binary_Backward.biadd  ~size ~nsw ~nuw
      | Binary{term=T2{tag=Bisub{size;nsw;nuw};a;b}} ->  ar2_binary_binary_binary dom a b x @@ B.Binary_Backward.bisub  ~size ~nsw ~nuw
  ...
  module MakeWorklist(Comp:sig 
    ...
    let add = Heap.add
    let pop_maximum = Heap.pop_maximum
  end
  ...
  let backward_propagate dom worklist (* constrain *) =
    let dom = ref dom in
    begin try 
        ...
        while true do
          let (Constraints.Any(max_elt) as max_any) = 
          ...
          (* Codex_log.feedback "backward propagation of %a %a"
           *   Constraints.pretty max_elt pretty !dom; *)
  ...
  let assume dom cond =
    assert(BooleanMap.find cond dom.boolean_map = Quadrivalent_Lattice.Top);
    let boolean_map = BooleanMap.replace cond Quadrivalent_Lattice.True dom.boolean_map in
    let dom = { dom with boolean_map } in
    let worklist = (BackwardWorklist.create 10) in
    BackwardWorklist.add worklist (Constraints.Any cond);
    let dom' = backward_propagate dom worklist in
