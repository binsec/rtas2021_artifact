...
module Make
    (Constraints:Constraints_constraints_sig.Constraints
     with type Condition.t = Cudd.Man.d Cudd.Bdd.t (* Temp. *))
...
struct
  ...
  module MakeConditionMap = Constraints_condition.CuddPIPartition.MakeConditionMap;;
  module type SConditionMap = Constraints_condition.CuddPIPartition.SConditionMap;;
  module ConditionMap = Constraints_condition.CuddPIPartition.ConditionMap;;
  module Condition = Constraints_condition.CuddPIPartition.Condition;;
  ...
  module BinaryConditionMap = struct
    module M = MakeConditionMap(B.Binary_Lattice);;
    include M
  ...
  let to_constraint x = x.condition
  ...
  let join a b = { condition = Constraints.Build.Boolean.(||) a.condition b.condition }
  ...
  module Binary_Forward = struct
    ...
    let biadd  ~size ~nsw ~nuw = ar2_binary_binary_binary ~sizea:size ~sizeb:size ~sizeres:size @@ B.Binary_Forward.biadd ~size ~nsw ~nuw
    let bisub  ~size ~nsw ~nuw = ar2_binary_binary_binary ~sizea:size ~sizeb:size ~sizeres:size @@ B.Binary_Forward.bisub ~size ~nsw ~nuw
  ...
  module Backward = struct
    ...
    module Binary = struct
      ...
      let biadd  ~size ~nsw ~nuw = ar2_binary_binary_binary ~sizea:size ~sizeb:size ~sizeres:size @@ B.Binary_Backward.biadd ~size ~nsw ~nuw
      let bisub  ~size ~nsw ~nuw = ar2_binary_binary_binary ~sizea:size ~sizeb:size ~sizeres:size @@ B.Binary_Backward.bisub ~size ~nsw ~nuw
    ...
    let backward_interp condition (Constraints.Any constr) =
      ...
      | Binary{term=T2{tag=Biadd{size;nsw;nuw};a;b}} ->  Binary.biadd  ~size ~nsw ~nuw condition a b constr
      | Binary{term=T2{tag=Bisub{size;nsw;nuw};a;b}} ->  Binary.bisub  ~size ~nsw ~nuw condition a b constr
  ...
  module MakeWorklist(Comp:sig 
    ...
    let create n = { heap = Heap.create n; hash = Hash.create n };;
    ...
    let pop_maximum {heap;hash} = 
      let any = Heap.pop_maximum heap in
    ...
    module Alt = struct
      ...
      let rec pop_maximum {heap;hash} =
        let any = Heap.maximum heap in
        try let cond = Hash.find hash any in
            Hash.remove hash any;
            (any,cond)
        with Not_found ->
          Heap.remove heap;
          pop_maximum {heap;hash}
      ;;
    end

    let (add,pop_maximum) =
      if option_worklist_merge_condition then (add,pop_maximum)
      else Alt.add,Alt.pop_maximum
    ;;
  ...
  let assume dom cond =
    ...
    assert (get_query_boolean cond (get_bdd dom) = Quadrivalent_Lattice.Top);
