...
module type Domain_S = sig
    module Constraints:Constraints_constraints_sig.Constraints
    ...
    (* Represent binary, integer and boolean dimensions in the abstract
       domain. *)
    type binary = Term_constructors_gadt.binary Constraints.t      
    type integer = Term_constructors_gadt.integer Constraints.t
    type boolean = Term_constructors_gadt.boolean Constraints.t
  
    val equal : t -> t -> bool
    val top : t
    val inter : t -> t -> t     (* MAYBE: we could also return None if the result is bottom. *)

    (* Express the path condition corresponding to the domain. Can be
       unknown if not known.  *)
    val to_constraint: t -> Term_constructors_gadt.boolean Constraints.t

    (* Projection to a non-relational basis. *)
    module Query:sig
      module Boolean_Lattice:Basis_sig.Boolean_Lattice with type t = Quadrivalent_Lattice.t
      module Integer_Lattice:Basis_sig.Integer_Lattice
      module Binary_Lattice:Basis_sig.Binary_Lattice                               
