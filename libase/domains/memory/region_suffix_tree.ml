...
module Make(Scalar:Domain_sig.Base) = struct
  ...
  struct
    ...
    let binary_empty = fun ~size ctx -> empty_suffix
    ;;

    (* No Top representable in this domain. *)
    let binary_unknown ~size:_ ~level:_ _ctx = assert false
    ...
    let binary_pretty ~size ctx fmt x = pp (Scalar.binary_pretty ~size ctx) fmt x
    ...
    let serialize ~size ctx a b acc =
      ...
      let rec suffix:type a. _ -> _ -> a Scalar.in_acc -> (_,a) Scalar.result =
        fun a b in_acc ->
          let Scalar.Result(inc_ofs,acc,d_offsets) = offset a.offsets b.offsets in_acc in
          let Scalar.Result(inc_ind,acc,d_indices) =
            indices a.indices b.indices (inc_ofs,acc) in
          Scalar.Result(inc_ofs && inc_ind, acc, fun tup ->
              let indices,tup = d_indices tup in
              let offsets,tup = d_offsets tup in
      ...
      and offset: type a. _ -> _ -> a Scalar.in_acc -> (_,a) Scalar.result =
        fun a b (included, acc) ->
        IntMap.fold_on_diff2 a b (Scalar.Result(included, acc, fun tup -> a,tup))
          (fun i sa sb (Scalar.Result(inc,acc,d_map) as res) ->
             match (sa,sb) with
        ...
        IntMap.fold_on_diff2 a b (Scalar.Result(included, acc, fun tup -> a,tup))
          (fun i sa sb (Scalar.Result(inc,acc,d_map) as res) ->
             ...
             | _ ->
               ...
               in
               let (ea,sa) = empty_if_none sa
               and (eb,sb) = empty_if_none sb in
               let Scalar.Result(inc,acc,d_suffix) = suffix sa sb (inc,acc) in
               let equal = ea == eb || Scalar.Binary.equal ea eb in
               let Scalar.Result(inc,acc,d_e) =
                 if equal then Scalar.Result(inc,acc,fun tup -> ea,tup)
                 else Scalar.serialize_binary ~size ctx ea eb (inc,acc)
               in
               Scalar.Result(inc, acc, fun tup ->
                   let e,tup = d_e tup in
                   let suffix,tup = d_suffix tup in
                   let map,tup = d_map tup in
    ...
    let nondet_binary ~size ctx v1 v2 =
      let Scalar.Result(_,tup,deserialize) =
        Scalar.serialize_binary ~size ctx v1 v2 (true,Scalar.empty_tuple)
      in
    ...
    let to_linear_exp ~size ctx x =
      let rec loop {leaf;offsets; indices} =
        ...
        let acc = IntMap.fold (fun k (e,x) acc ->
            ...
            (Scalar.Binary_Forward.biadd ~size ~nsw:false ~nuw:true ctx ke @@ loop x)::acc
          ) indices acc in
        let acc = IntMap.fold (fun k x acc ->
            let k = Scalar.Binary_Forward.biconst ~size (Z.of_int k) ctx in
            (Scalar.Binary_Forward.biadd ~size ~nsw:false ~nuw:true ctx k @@ loop x)::acc
          ) offsets acc in
        match acc with
        | [a] -> a
    ...
    let bisub ~size ctx a b =
      ...
      Scalar.Binary_Forward.bisub ~size ~nsw:true ~nuw:false ctx ba bb
    ;;


    let rec assume ~size ctx cond {leaf;offsets;indices;min;max} =
      {leaf;
       offsets = IntMap.map (assume ~size ctx cond) offsets;
       min; max;
       indices = indices |> IntMap.map (fun (e,s) ->
           (Scalar.assume_binary ~size ctx cond e,
            assume ~size ctx cond s
           ) )
      }
    ;;
        ...
        let new_indices =
          ...
          | (e1, s1) ->
            if leaf then begin
              ...
              let add = Scalar.Binary_Forward.biadd ~size ~nsw:true ~nuw:false ctx with_zero e in
              ...
              if s1 == only_leaf_no_bound then
                ...
                let add = Scalar.Binary_Forward.biadd ~size ~nsw:true ~nuw:false ctx e1 e in                
                ...
                if ( IntMap.is_empty s1.offsets && IntMap.is_empty s1.indices) then begin
                  ...
                  let _add = Scalar.Binary_Forward.biadd ~size ~nsw:true ~nuw:false ctx e1 e in
    ...
    let within_bounds ~size ctx off =
      ...
      let module Bool = struct
        ...
        let nondet2_bool v1 v2 =
          let Scalar.Result(_,tup,deserialize) =
            Scalar.serialize_boolean ctx v1 v2 (true,Scalar.empty_tuple)
          in
          let res_tup = Scalar.typed_nondet2 ctx tup in
          let res,_ = deserialize res_tup in
          res
        ;;
  ...
  module type Array_Arg = sig
    ...
    val load: memsize:int -> offset:int -> size:int -> Scalar.Context.t -> t -> value (* Value.binary *)
    
    (* Copying part of a cell. *)
    val extract: memsize:int -> offset:int -> size:int -> Scalar.Context.t -> t -> t

    val serialize:memsize:int -> Scalar.Context.t -> t -> t -> 'a Scalar.in_acc -> (t,'a) Scalar.result
        
    val pretty:memsize:int -> Scalar.Context.t -> Format.formatter -> t -> unit

    val union: memsize:int -> Scalar.Context.t -> t -> t -> t

    val nconcat: Scalar.Context.t -> (int * t) list -> t
  ...
  module type Array_result = sig
    type t
    type cell
    type value                  (* Probably temporary, to be replaced by extract? *)

    val pretty:memsize:int -> Scalar.Context.t -> Format.formatter -> t -> unit

    (* Converts a cell into an array with a single cell whose
       contents is cell. *)
    val singleton: memsize:int -> Scalar.Context.t -> cell -> t
    ...
    (* Probably temporary. *)
    val load: memsize:int -> offset:int -> size:int -> Scalar.Context.t -> t -> value

    (* Extract a cell from an array. *)
    val extract: memsize:int -> offset:int -> size:int -> Scalar.Context.t -> t -> cell

    val serialize:memsize:int -> Scalar.Context.t ->
      t -> t -> 'a Scalar.in_acc ->
      (t,'a) Scalar.result
  ...
  module Make_Array2(Value:Value with module Scalar = Scalar)(Cell:Array_Arg with type value = Value.binary):Array_result
    with type cell = Cell.t
     and type value = Cell.value
  = struct
    ...
    let serialize ~memsize ctx a b acc =
      ...
      let Scalar.Result(inc,acc,d) = Cell.serialize ~memsize:newsize ctx a.cell_contents b.cell_contents acc
      in Scalar.Result(inc, acc, (fun tup -> let x,tup = d tup in {cell_size=newsize;cell_contents=x},tup))      
    ;;
  ...
  module Memory(Value:Value with module Scalar = Scalar)() = struct
    module Scalar = Scalar
    module Value = Value 
    ...
    let join_values_nondet ~size ctx v1 v2 =
      let Scalar.Result(_,tup,deserialize) =
        Value.serialize ~size ctx v1 v2 (true, Scalar.empty_tuple)
      in
      let res_tup = Scalar.typed_nondet2 ctx tup in
      let res,_ = deserialize res_tup in
      res
    ...
    let join_values_union ~size ctx v1 v2 =
      ...
      let res_tup = Scalar.union condition ctx tup in
      let res,_ = deserialize res_tup in
      res
    ;;
    ...
    module Make_Array(Cell:Array_Arg with type value = Value.binary)():
    Array_result with type cell = Cell.t
                  and type value = Cell.value = struct
    ...
    module rec Cell:sig
      ...
      let serialize ~memsize ctx a b acc =
        ...
        match a,b with
        | Value a, Value b ->
          let Scalar.Result(inc,acc,d) = Value.serialize ~size:(memsize * 8) ctx a b acc in
          Scalar.Result(inc, acc, (fun tup -> let x,tup = d tup in Value x,tup))
        | Struct a, Struct b ->
          let Scalar.Result(inc,acc, d) = Struct.serialize ~memsize ctx a b acc in
          Scalar.Result(inc, acc, (fun tup -> let x,tup = d tup in Struct x,tup))
        | Array a, Array b ->
          let Scalar.Result(inc,acc, d) = Array.serialize ~memsize ctx a b acc in
          Scalar.Result(inc, acc, (fun tup -> let x,tup = d tup in Array(x),tup))
        (* Decay into an array. *)
        | Value a, Array b ->
          let Scalar.Result(inc, acc, d) =
            Array.serialize ~memsize ctx (Array.singleton ~memsize ctx @@ Value a) b acc in
          Scalar.Result(inc, acc, (fun tup -> let res,tup = d tup in Array res,tup))
        | Array a, Value b ->
          let Scalar.Result(inc, acc, d) =
            Array.serialize ~memsize ctx a (Array.singleton ~memsize ctx @@ Value b) acc in
          Scalar.Result(inc, acc, (fun tup -> let res,tup = d tup in Array res,tup))
        (* Decay into a struct. *)
        | (Value _ | Array _) as a, Struct b ->
          let Scalar.Result(inc, acc, d) =
            Struct.serialize ~memsize ctx (Struct.singleton ~memsize a) b acc in
          Scalar.Result(inc, acc, (fun tup -> let res,tup = d tup in Struct res,tup))
        | Struct a, ((Value _ | Array _) as b) ->
          let Scalar.Result(inc, acc, d) =
            Struct.serialize ~memsize ctx a (Struct.singleton ~memsize b) acc in
          Scalar.Result(inc, acc, (fun tup -> let res,tup = d tup in Struct res,tup))
        ;;
      ...
      let union ~memsize ctx m1 m2 =
        let Scalar.Result(_,tup,deserialize2) = serialize ~memsize ctx m1 m2 (true, Scalar.empty_tuple) in
        let level = Scalar.Context.level ctx in
        let res_tup = Scalar.union (Transfer_functions.Condition.fresh ~level) ctx tup in
        let res,_ = deserialize2 res_tup in res
      ;;
    ...
    and Struct:sig
      ...
      val store: memsize:int -> offset:int -> size:int -> Scalar.Context.t -> t -> Cell.t -> t
      val load: memsize:int -> offset:int -> size:int -> Scalar.Context.t -> t -> Value.binary

      val pretty:memsize:int -> Scalar.Context.t -> Format.formatter -> t -> unit

      (* Copying part of a struct. *)
      val extract: memsize:int -> offset:int -> size:int -> Scalar.Context.t -> t -> Cell.t
      ...
      module ExtractedCell:sig
        ...
        val extract :
          memsize:int ->
          offset:int -> size:int -> Scalar.Context.t -> Cell.t -> t

        val serialize:memsize:int -> Scalar.Context.t -> t -> t ->
          'a Scalar.in_acc -> (Cell.t,'a) Scalar.result
      ...
      let serialize ~memsize ctx a b (inc, acc) =
        ...
        IntervalMap.fold_on_diff a b (Scalar.Result(inc, acc,fun tup -> a,tup)) ~extract:(mk_extract ctx)
          (fun ~size offset a b (Scalar.Result(inc,acc,d_map)) ->
             let Scalar.Result(inc,acc,d_cell) = ExtractedCell.serialize ~memsize:size
              ctx a b (inc,acc) in
             Scalar.Result(inc,acc,fun tup ->
                 let cell,tup = d_cell tup in
                 let map,tup = d_map tup in
    ...
    and Array:sig
      include Array_result with type cell := Cell.t and type value := Value.binary
    end =
      Make_Array(struct include Cell type value = Value.binary end)()
    ...
    let serialize ctx a b acc =
      assert(a.size == b.size);
      let Scalar.Result(inc,acc,deserialize) =
        Cell.serialize ~memsize:a.size ctx a.contents b.contents acc
      in Scalar.Result(inc, acc, (fun tup -> let contents,tup = deserialize tup in
    ...
    module type PtrInteraction = sig
      ...
      val load: min:Bound.t -> max:Bound.t ->
        memsize:int -> offset:int -> size:int -> Scalar.Context.t ->
        Cell.t -> address -> Value.binary

      val store: memsize: int -> offset:int -> size:int ->
        min:Bound.t -> max:Bound.t ->
        Scalar.Context.t ->
        Cell.t -> address -> Value.binary -> Cell.t
    end
    ...
    module type Indices = sig
      ...
      val load: min:Bound.t -> max:Bound.t ->
        memsize:int -> offset:int -> size:int -> Scalar.Context.t ->
        Scalar.binary -> int -> address -> Cell.t -> Value.binary
      
      val store: min:Bound.t -> max:Bound.t ->
        memsize:int -> offset:int -> size:int -> Scalar.Context.t ->
        Scalar.binary -> int -> address -> Cell.t -> Value.binary -> Cell.t
    end
            ...
            let acc = IntMap.fold (fun k (e,suffix) acc ->
                ...
                | false -> begin
                    ...
                    (* Cannot do it: suffix.min is relative to the offset. We forget it for now.
                       let (min,max)= Bound.inter (min,max) (suffix.min,suffix.max) in *)
                    let esub =
                      Scalar.Binary_Forward.biadd ~size:ptrsize ~nsw:true ~nuw:false ctx esub @@
                      Scalar.Binary_Forward.bimul ~size:ptrsize ~nsw:true ~nuw:false ctx e @@
                      Scalar.Binary_Forward.biconst ~size:ptrsize (Z.of_int k) ctx
                    in
                    try
    ...
    module MakeIndicesInPlaceEnumeration(PtrInteraction:PtrInteraction):Indices = struct
      ...
      let store ~min ~max ~memsize ~offset ~size ctx e k suffix mem value =
    ...
    let store ~size ctx {size=memsize;contents} address value =
      let contents =
        PtrInteraction.store ~memsize ~offset:0 ~size:(size / 8)
          ~min:(Bound.CBound 0)
          ~max:(Bound.CBound memsize)
          ctx contents address value
      in
      {size=memsize;contents}

    let _store ~size ctx {size=memsize;contents} address value =
      let res = store ~size ctx {size=memsize;contents} address value in
    ...
    let load ~size ctx {size=memsize;contents} address =
      PtrInteraction.load
        ~min:(Bound.CBound 0)
        ~max:(Bound.CBound memsize)
        ~memsize ~offset:0 ~size:(size / 8) ctx contents address
