...
module Make (Sub : Subdomain) = struct
  ...
  (* We could use an array where each bit would be a bit in another variable, but this would
     be slow for most operation. So, we use this, which is complemented with the known 0 bits
     and known 1 bits. *)
  type binary_part =
    ...
    | Extract of { size:int;
                   index:int;
                   oldsize:int;
                   value:Sub.binary } (* Could be an expression in a normal form (e.g. ANF). *)
    (* XXX: j'ai un besoin de concatener des zeros ensemble (et toutes les constantes en general).
     * Mais je peux utiliser les queries pour faire ca.           *)


    (* | Constant of {size:int; value:Z.t}          (\* Not really necessary. *\) *)
    (* | Repeat of {repeat:int; value:Sub.binary }     *)
    (* OK, let's just loose information when we repeat. *)
 (* Repeat a single bit; happens when unknown bsext. *)
...
module Part = struct
  ...
  module Parts = struct
    ...
    (* TODO: fold on common intervals; and maybe simultaneously find
       the values for 0 and 1 on this interval. *)
    ...
    (* XXX: append les constantes ensembles, mais aussi les morceaux extraits pour les faire se recoller.
     * Sachant qu'il suffit de regarder les elements a l'intrersection, si tout est maximalement concatene *)
    ...
    let fold2 ctx f acc pa pb =
      let rec loop acc pa pb = match pa,pb with
        | [],[] -> acc
        | [], _ | _, [] -> assert false (* Different lengths. *)
  ...
  (* module PartsAlt = struct
   *
   *   (\* An alternative, that don't use any storage when it is not
   *      required, which is often the case. To use with GADT to avoid
   *      checking on useless cases.  *\)
   *   (\* Alternatively, just use emptylist to encode the case where
   *      parts has nothing useful to say; but this takes more memory
   *      than this ad-hoc list. *\)
   *   (\* | SeeComplete               (\\* No more information than complete has. *\\)
   *    * | Last                      (\\* No element *\\)
   *    * | ConsVariable of {....; next} *\)
   *
   *
   * end *)

  (* We store both the parts and the complete version:
     - The complete version is often needed;
     - We use the computation of the complete version to retrieve which elements are 0 or 1. *)
  (* XXX: je pourrais avoir unbitvector contenant la polarité des bits, comme 1 et 0.
     0 = positive, je garde le bit du sub telquel.
     1 = negative, le resultat c'est l'inverse du bit sub.

     Utile pour les mixed-boolean arithmetic operations.  *)
  ...
  module Binary = struct
    ...
    let lift2_pred ~size ctx f x y =
      ...
      let open Basis_sig in 
      match Sub.Query.convert_to_quadrivalent @@ Sub.Query.boolean ctx with_parts with
  ...
  module Memory_Forward = struct
    ...
    let load ~size ctx mem addr =
      Binary.lift0 ~size @@ Sub.Memory_Forward.load ~size ctx mem addr.complete;;
  ...
  module Binary_Forward = struct
    ...
    let bisub ~size ~nsw ~nuw ctx a b = Binary.lift0 ~size @@ SBF.bisub ~size ~nsw ~nuw ctx a.complete b.complete
    ...
    let biadd ~size ~nsw ~nuw ctx a b =
      ...
      in
      if orable then
        let complete = SBF.biadd ~size ~nsw ~nuw ctx a.complete b.complete in
        bor' complete ~size ctx a b
      else
        Binary.lift0 ~size @@ SBF.biadd ~size ~nsw ~nuw ctx a.complete b.complete
  end

  let binary_unknown ~size ~level ctx = Binary.lift0 ~size @@ Sub.binary_unknown ~size ~level ctx;;
  let boolean_unknown = Sub.boolean_unknown
  let integer_unknown = Sub.integer_unknown

  let assume_integer = Sub.assume_integer
  let assume_boolean = Sub.assume_boolean
  let assume_binary ~size ctx c b =
    (* Note: lazy assume would perform better for recognizing equality. *)
    Binary.lift1 ~size (fun ~size b -> Sub.assume_binary ~size ctx c b) b
  ;;
  let assume_memory = Sub.assume_memory
  ...
  let serialize_binary ~size ctx a b (included, acc) =
    let Sub.Result (included, in_tup, deserialize) = Sub.serialize_binary ~size ctx a.complete b.complete (included, acc) in
    Result (included, in_tup, (fun out_tup ->
        let res, out_tup = deserialize out_tup in
  ...
  (**************** Queries ****************)
   ...
   module Query = struct
    ...
    (* let binary_is_singleton_ ~size ctx x = binary_is_singleton_ ~size ctx x.complete;; *)
...
= struct
  ...
  let typ ~size ctx x = D.typ ~size ctx x.complete

  let with_type typ ~size ctx x =
    Binary.lift0 ~size @@ D.with_type typ ~size ctx x.complete

  let global_symbol ctx name =
    let size,v = D.global_symbol ctx name in
    size, Binary.lift0 ~size v
