...
module type Constraints = sig
  ...
  type 'a complete_term = private
    ...
    | Mu_formal:{level:level; actual: ('res t * Term_constructors_gadt.boolean t) } -> 'res complete_term (* if level != 0; intro != None *)
  ...
  module Build:sig
    ...
    module Boolean:sig
      ...
      include Transfer_functions.Boolean_Forward
        with module Arity := Basis_sig.Pure_Arity.Forward
    ...
    module Mu_Formal:sig
      val intro: level:int -> actual:'a t ->  actual_cond: Term_constructors_gadt.boolean t -> 'a Term_constructors_gadt.typ -> 'a t
    end
    
    module Tuple:sig
      val get_binary: size:int -> int -> tuple ->  Term_constructors_gadt.binary t      
      val get_integer:int -> tuple ->  Term_constructors_gadt.integer t
      val get_boolean:int -> tuple ->  Term_constructors_gadt.boolean t
      val nondet: level:int -> conda_bool:Term_constructors_gadt.boolean t ->  a:tuple ->
        condb_bool:Term_constructors_gadt.boolean t -> b:tuple -> tuple
      val mu: level:int -> init:tuple -> var:tuple -> body:tuple -> body_cond:Term_constructors_gadt.boolean t -> tuple
    end
