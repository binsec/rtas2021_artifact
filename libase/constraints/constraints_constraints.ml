...
(* Note: Cannot use any (because the boxing clashes with using Weak),
  ...
  let compare: type a b. a t -> b t -> int = fun a b ->
    Pervasives.compare (get_id_int a) (get_id_int b)
 ...
 (* Relies on the fact that for all variants, the position is the same in memory. *)
  ...
  module Any = struct
    ...
    let compare a b = Pervasives.compare (get_id_int a) (get_id_int b)
  end
  ...
  module Pretty = struct
    ...
    = struct
      ...
      module AnyWeak = Weak.Make(Any)
      module TupleWeak = Weak.Make(Tuple)

      (* Ensures that definitions are printed only once. *)
      let printed = AnyWeak.create 17;;
      let printed_tuple = TupleWeak.create 17;;

      let rec pretty_definition: type a. Format.formatter -> a t -> unit = fun fmt cons ->
        if not @@ AnyWeak.mem printed (Any cons)
        then begin
          AnyWeak.add printed (Any cons);
          match cons with
      ...
      and pretty_definition_tuple fmt tup =
        if not @@ TupleWeak.mem printed_tuple tup
        then begin
          TupleWeak.add printed_tuple tup;
          match tup with
  ...
  module HashCons = struct
    ...
    module HashBool = Ephemeron.K1.Make(struct
        type t = boolean complete_term
        include Param
      end);;

    module HashInt = Ephemeron.K1.Make(struct
        type t = integer complete_term
        include Param
      end);;

    module HashBin = Ephemeron.K1.Make(struct
        type t = binary complete_term
        include Param
      end);;
...
(* Similar to Hashcons, except that we do not need to hashcons because
  ...
  module Build = struct
    ...
    module Binary = struct
      ...
      let biadd  ~size ~nsw ~nuw =
        ar2_binary_binary_binary_commutative ~size (TCBB.biadd ~size ~nsw ~nuw)
      let bisub  ~size ~nsw ~nuw =
        ar2_binary_binary_binary ~size (TCBB.bisub ~size ~nsw ~nuw)      
