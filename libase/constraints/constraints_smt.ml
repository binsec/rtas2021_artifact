...
let option_translate_binary_to_integer = true
...
module Make
    ...
    (S:Smtbackend_smtlib_sig.Untyped_S) = struct
  ...
  struct
    ...
    let translate: type res arg. res C.t -> (arg,res) term -> arg t -> S.value =
      fun constr term arg ->
        ...
        (* Normal translation. *)
        let normal: (arg,res) term * arg t -> S.value = function
...
module MakeFirstOrder
    (C:Constraints_constraints.Constraints)    
    (S:Smtbackend_smtlib_sig.Untyped_S)
...
= struct
  ...
  and MakeApply:sig
    val translate: 'res C.t -> ('arg,'res) Term_constructors_gadt.term -> 'arg M.t -> S.value
  end = M.Make(MakeArg)
   ...
   let translate assertion = 
     ...
     | S.Sat -> Domain_sig.Sat () 
     | S.Unsat -> Domain_sig.Unsat
     | S.Unknown -> Domain_sig.Unknown
end
...
module MakeHorn
    (C:Constraints_constraints.Constraints)    
    (S:Smtbackend_smtlib_sig.Untyped_Muz)
